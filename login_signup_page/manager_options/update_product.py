...
def load():
    ...
    if data != []:
        ...
        return got_id, name, count, buy_price, low_sell_price, sell_price, trader, date_of_buy
load()
...
def update():
    ...
    return (got_id, name_entry.get(), count_entry.get(), buy_price_entry.get(), low_sell_price_entry.get(), sell_price_entry.get(), trader_entry.get(), date_of_buy_entry.get())
...
global save_btn
save_btn = Button(root, text='حفظ', bg='#ABB2B9', fg='#117A65', font=('tajawal', 12, 'bold'), bd=2, relief='solid', command=save1)
